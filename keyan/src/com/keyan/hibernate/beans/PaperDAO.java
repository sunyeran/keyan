...
public class PaperDAO extends HibernateDaoSupport implements IPaperDAO   {
	...
	public List<Paper> findAllForPage(String queryString,int pageSize,int page){
		Session session = getSession();
		List<Paper> list = session.createQuery(queryString)
				.setFirstResult(page)
	...
	public int getCoPaperCount(String hql,String user,String begindate,String enddate)
		...
		ParsePosition pos2 = new ParsePosition(0);
		final Date date2 = sdf.parse(enddate,pos2);
		Long count = (Long)getHibernateTemplate().find(hql, new Object[]{user,user,date1,date2}).listIterator().next();
		return (int) count.intValue();
	...
	@SuppressWarnings("unchecked")
	public List<Paper> findAll(String str) {
		List<Paper> list=getHibernateTemplate().find(str);
		return list;
	}
	public List findPaper(String str,String paperName) {
		...
		try {
			...
			return paperlist;*/
			str=str+"\""+paperName+"\"";
			List list = session.createSQLQuery(str).addEntity(Paper.class).list();
			releaseSession(session);
			return list;
		} catch (RuntimeException re) {
			log.error("find all failed", re);
