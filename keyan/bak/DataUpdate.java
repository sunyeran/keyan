	...
	public void service(HttpServletRequest request ,
		...
		paperDAO = (PaperDAO) ctx.getBean("paperDAO");
		  for(int i=0;i<jsonArray.size(); i++){
			...
			paper.setPaper_id(paper.getPaper_id());
			...
			patentDAO = (PatentDAO) ctx.getBean("patentDAO");
			  for(int i=0;i<jsonArray.size(); i++){
				JSONObject jsonJ = jsonArray.getJSONObject(i);
				if(jsonJ.containsKey("patent_id")){
					   patent.setPatent_id(new Integer(jsonJ.getString("patent_id")));
					 }
				System.out.println(patent.getPatent_id());
				patent.setPatent_id(patent.getPatent_id());
				patent.setName(name);
				...
				patent.setApply_no(jsonJ.getInt("apply_no"));
			...
			subjectDAO = (SubjectDAO) ctx.getBean("subjectDAO");
			  for(int i=0;i<jsonArray.size(); i++){
				...
				subject.setAppraisalDept(jsonJ.getString("appraisalDept"));
			...
			teachingDAO = (TeachingDAO) ctx.getBean("teachingDAO");
			  for(int i=0;i<jsonArray.size(); i++){
				...
				teaching.setWords(jsonJ.getInt("words"));
				try {
